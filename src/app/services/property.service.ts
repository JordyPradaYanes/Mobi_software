...
export class PropertyService {
  ...
  getUserPropertiesCount(userId: string): Observable<number> {
    const q = query(this.propertiesCollection, where('userId', '==', userId));
    return from(getDocs(q)).pipe(
      map((snapshot) => snapshot.size),
      catchError((error) => {
        console.error(
          `Error getting user properties count for user ${userId}:`,
          error
        );
        return of(0);
      })
  ...
  // Obtener el conteo total de propiedades en el sistema
  getTotalPropertiesCount(): Observable<number> {
  ...
  getProperties(): Observable<Property[]> {
    return from(getDocs(this.propertiesCollection)).pipe(
      map((snapshot) =>
        snapshot.docs.map((doc) => ({ id: doc.id, ...doc.data() } as Property))
      ),
      catchError((error) => {
  ...
  getPropertiesByIds(ids: string[]): Observable<Property[]> {
    ...
    return forkJoin(observables).pipe(
      map(
        (properties) => properties.filter((p) => p !== undefined) as Property[]
      ),
      tap((properties) =>
        console.log(
          `Fetched ${properties.length} properties for IDs: ${ids.join(', ')}`
        )
      )
    );
  }
